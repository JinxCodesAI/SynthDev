...
class CalculateTool extends BaseTool {
    constructor() {
        super('calculate', 'Perform mathematical calculations including basic arithmetic, trigonometry, and advanced functions');
    ...
    async implementation(params) {
        ...
        if (typeof precision !== 'number' || precision < 0 || precision > 15) {
            return this.createErrorResponse(
                'Precision must be a number between 0 and 15',
                { expression, precision, valid_range: '0-15' }
            );
        }
        ...
        try {
            ...
            if (!allowedChars.test(cleanExpression)) {
                return this.createErrorResponse(
                    ...
                    { expression, clean_expression: cleanExpression }
                );
            }
            ...
            if (typeof result !== 'number') {
                return this.createErrorResponse(
                    ...
                    { expression, result_type: typeof result }
                );
            }
            
            if (isNaN(result)) {
                return this.createErrorResponse(
                    'Expression resulted in NaN',
                    { expression }
                );
            }
            ...
            if (!isFinite(result)) {
                return this.createErrorResponse(
                    'Expression resulted in infinity',
                    { expression }
                );
            }
