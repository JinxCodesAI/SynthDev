...
class ExecuteScriptTool extends CommandBaseTool {
    constructor() {
        super('execute_script', 'Execute a JavaScript script as a child process for calculations, text transformations, and data aggregation');
...
Mark as SAFE if script only uses allowed operations. Mark as UNSAFE only if it uses forbidden operations. Be precise and focus on actual security violations.`;
            ...
            safetyResult.tokens_used = response.usage ? (response.usage.prompt_tokens + response.usage.completion_tokens) : 0;
            ...
            logger.warn('AI safety assessment failed, falling back to pattern matching:', error.message);
    ...
    performBasicSafetyCheck(script) {
        ...
        const dangerousPatterns = [
            ...
            { pattern: /fs\.write|fs\.append|fs\.create|fs\.mkdir|fs\.rmdir|fs\.unlink|fs\.rename|fs\.copy/i, reason: 'File modification operations not allowed' },
            { pattern: /writeFile|appendFile|createWriteStream/i, reason: 'File writing operations not allowed' },
            ...
            // Network operations
            { pattern: /http|https|net|dgram|tls|url\.request/i, reason: 'Network operations not allowed' },
            { pattern: /fetch|XMLHttpRequest|WebSocket/i, reason: 'Network requests not allowed' },

            // Dangerous globals and eval
            { pattern: /eval\s*\(|Function\s*\(|setTimeout|setInterval/i, reason: 'Dynamic code execution not allowed' },
            { pattern: /global\.|globalThis\.|process\.env/i, reason: 'Global object access restricted' },

            // Module loading restrictions
            { pattern: /require\s*\(\s*['"`][^'"`]*(?:child_process|fs|http|https|net|crypto)[^'"`]*['"`]\s*\)/i, reason: 'Restricted module access' },

            // Potential infinite loops (basic detection)
            { pattern: /while\s*\(\s*true\s*\)|for\s*\(\s*;\s*;\s*\)/i, reason: 'Potential infinite loop detected' }
        ];
        ...
        return {
            ...
            recommendations: safetyIssues.length > 0 ? ['Remove dangerous operations and try again'] : [],
    ...
    async implementation(params) {
        const { script, timeout = 10000, costsManager } = params;
        
        // Validate timeout
        if (timeout < 1000 || timeout > 30000) {
            return this.createErrorResponse(
                'Timeout must be between 1000 and 30000 milliseconds',
                { timeout, valid_range: '1000-30000' }
            );
        }
        ...
        if (!safetyCheck.safe) {
            return this.createErrorResponse(
                'Script failed AI safety validation',
                {
        ...
        const tempScriptPath = join(process.cwd(), `temp_script_${Date.now()}_${Math.random().toString(36).substring(2, 11)}.js`);
                ...
                child.on('close', (code) => {
                    if (!killed) {
                        ...
                        try {
                            ...
                            const logger = getLogger();
                            logger.warn('Failed to cleanup temp script file:', cleanupError.message);
                        }
