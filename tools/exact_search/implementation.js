...
class ExactSearchTool extends BaseTool {
    constructor() {
        super('exact_search', 'Searches for exact occurrences of a given string in all files listed by list_directory tool, returning matching fragments with context lines');
    ...
    async implementation(params) {
        ...
        if (search_string.length === 0) {
            return this.createErrorResponse(
                'search_string cannot be empty',
                { search_string, length: search_string.length }
            );
        }
        ...
        try {
            ...
            try {
                ...
                return this.createErrorResponse(
                    `Failed to scan directory: ${scanError.message}`,
                    { root_path: rootPath, error_code: scanError.code }
                );
            }
            ...
            return this.createErrorResponse(
                `Unexpected error during search: ${error.message}`,
                { search_string, stack: error.stack }
            );
        }
    }
}
