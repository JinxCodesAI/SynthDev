...
class ExplainCodebaseTool extends CommandBaseTool {
  constructor() {
    super('explain_codebase', 'Provides AI-generated explanations in markdown format for natural language questions about the indexed codebase summaries from ".index/codebase-index.json"');
...
`;
  ...
  async implementation(params) {
    ...
    if (!question || typeof question !== 'string' || question.trim() === '') {
      return this.createErrorResponse('question parameter is required and must be a non-empty string');
    }
    ...
    if (!userPrompt) {
      return this.createErrorResponse('Failed to construct prompt from codebase index summaries');
    }
    ...
    try {
      const modelConfig = config.hasFastModelConfig() ? config.getModel('fast') : config.getModel('base');
      ...
      // Check for errors
      if (responseError) {
        return this.createErrorResponse('AI processing failed', { error: responseError.message });
      }
      ...
      return this.createErrorResponse('Failed to initialize AI client or process request', { error: error.message });
    }
  }
}
