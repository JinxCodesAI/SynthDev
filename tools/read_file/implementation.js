...
class ReadFileTool extends FileBaseTool {
    constructor() {
        super('read_file', 'Read the contents of a file from the file system using a relative path');
        ...
        try {
            ...
            if (typeof start_line === 'number' || typeof end_line === 'number') {
                ...
                if (end_line > lines.length) {
                    return this.createErrorResponse(
                        'end_line is beyond the end of the file',
                        { file_path, end_line, total_lines: lines.length }
                    );
                }
                const end = (typeof end_line === 'number' && end_line >= start && end_line <= lines.length) ? end_line : lines.length;
